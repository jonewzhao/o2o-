...
from math import ceil  #向下取整
import pandas as pd


def get_source_data():
    DataPath = 'data'
    #parse_dates读取有时间序列的列，列名为Date_received以及Date
    offtrain  = pd.read_csv('data\ccf_offline_stage1_train.csv',parse_dates=['Date_received','Date'])
    offtrain.columns = ['User_id', 'Merchant_id', 'Coupon_id', 'Discount_rate', 'Distance', 'Date_received', 'Date']

    ontrain = pd.read_csv('data\ccf_online_stage1_train.csv',parse_dates=['Date_received','Date'])
    ontrain.columns = ['User_id','Merchant_id','Action','Coupon_id','Discount_rate','Date_received','Date']

    offtest = pd.read_csv('data\ccf_offline_stage1_test_revised.csv',parse_dates=['Date_received'])
    offtest.columns = ['User_id', 'Merchant_id', 'Coupon_id', 'Discount_rate', 'Distance', 'Date_received']

    print(offtrain.info())
    print(offtrain.head(5))
    print(ontrain.info())
    print(ontrain.head(5))
    return offtrain,ontrain,offtest
#
#get_source_data()
...
def null_process_offline(dataset,predict = False):
    ...
    #满减 discount_rate_x表示满多少，discount_rate_y表示减多少
    dataset[['discount_rate_x','discount_rate_y']] = dataset[dataset.Discount_rate.str.contains(':')==True]['Discount_rate'].str.split(":",expand = True).astype(int)
    #折扣率
...
#生成交叉训练集
def data_process(offtrain,ontrain,offtest):
    #交叉集1：收到券的日期大于4月14日-小于5月14日
    time_range = ['2016-04-16','2016-05-15']
    dataset1 = offtrain[(offtrain.Date_received>=time_range[0])&(offtrain.Date_received<=time_range[1])].copy()
    dataset1['label'] = 0
    dataset1.loc[
        (dataset1.Date!=date_null)&(dataset1.Date-dataset1.Date_received <=datetime.timedelta(15)),'label']=1
    #交叉训练集一特征offline：线下数据中领券日期于1月1日和小于4月1日，用券日期在1月1号到-4月15号
    time_range_date_received = ['2016-01-01','2016-04-01']
    time_range_date = ['2016-01-01','2016-04-15']
    feature1_off = offtrain[(offtrain.Date>=time_range_date[0])&(offtrain.Date<=time_range_date[1])|
                            ((offtrain.Coupon_id==0)&(offtrain.Date_received>=time_range_date_received[0])&(offtrain.Date_received<=time_range_date_received[1]))]

    #交叉训练集1特征online:线上数据中领券和使用权的日期大于1月1日小于4月15日

    feature1_on = ontrain[(ontrain.Date>=time_range_date[0])&(ontrain.Date<=time_range_date[1])|
                          ((ontrain.Coupon_id==0)&(ontrain.Date_received >=time_range_date_received[0])&(ontrain.Date_received<=time_range_date_received[1]))]

    #交叉训练集2：收到券的日期大于5月15日和小于6月15日
    time_range = ['2016-05-16','2016-06-15']
    dataset2 = offtrain[(offtrain.Date_received>=time_range[0])&(offtrain.Date_received<=time_range[1])].copy()
    dataset2['label'] = 0
    dataset2.loc[
    ...
    feature2_off = offtrain[(offtrain.Date >= time_range_date[0]) & (offtrain.Date <= time_range_date[1]) | (
            (offtrain.Coupon_id == 0) & (offtrain.Date_received >= time_range_date_received[0]) & (
            offtrain.Date_received <= time_range_date_received[1]))]
    # 交叉训练集二特征online：线上数据中领券和用券日期大于2月1日和小于5月14日
    feature2_on = ontrain[(ontrain.Date >= time_range_date[0]) & (ontrain.Date <= time_range_date[1]) | (
            (ontrain.Coupon_id == 0) & (ontrain.Date_received >= time_range_date_received[0]) & (
            ontrain.Date_received <= time_range_date_received[1]))]

    #测试集
    dataset3 = offtest
    # 测试集特征offline :线下数据中领券和用券日期大于3月15日和小于6月30日的
    time_range = ['2016-03-16', '2016-06-30']
    feature3_off = offtrain[((offtrain.Date >= time_range[0]) & (offtrain.Date <= time_range[1])) | (
            (offtrain.Coupon_id == 0) & (offtrain.Date_received >= time_range[0]) & (
            offtrain.Date_received <= time_range[1]))]
    # 测试集特征online :线上数据中领券和用券日期大于3月15日和小于6月30日的
    feature3_on = ontrain[((ontrain.Date >= time_range[0]) & (ontrain.Date <= time_range[1])) | (
            (ontrain.Coupon_id == 0) & (ontrain.Date_received >= time_range[0]) & (
            ontrain.Date_received <= time_range[1]))]
...
def get_features(dataset,featureoff,featureon):
    dataset = get_offline_features(dataset,featureoff)
    return get_online_features(featureon,dataset)
...
def get_offline_features(X,offline):
    ...
    coupon_consume = temp[temp.Date!=date_null]  #优惠券消费
    coupon_noconsume = temp[temp.Date==date_null] #优惠券不消费

    user_coupon_consume = coupon_consume.groupby('User_id')


    X['weekday']=X.Date_received.dt.weekday #收到优惠券时星期几
    X['day'] = X.Date_received.dt.day       #收到优惠券时哪一天
    ...
    #用户的优惠券不消费次数
    temp = coupon_noconsume.groupby('User_id').size().reset_index(name='u3')
    X = pd.merge(X,temp,how='left',on = 'User_id')
    ...
    #正常消费平均间隔
    temp = pd.merge(temp, temp.groupby('User_id').Date.max().reset_index(name='max')) #最大时间
    temp = pd.merge(temp, temp.groupby('User_id').Date.min().reset_index(name='min')) #最小时间
    temp = pd.merge(temp, temp.groupby('User_id').size().reset_index(name='len'))     #消费次数
    ...
    temp = temp.drop_duplicates('User_id')   #去除重复的用户数据
    ...
    # 不同优惠券不使用次数
    temp = coupon_noconsume.groupby(['User_id', 'Discount_rate']).size().reset_index(name='u43')
    X = pd.merge(X, temp, how='left', on=['User_id', 'Discount_rate'])
    ...
    # 商家优惠券被领取后不核销次数
    temp = coupon_noconsume.groupby('Merchant_id').size().reset_index(name='m7')
    X = pd.merge(X, temp, how='left', on='Merchant_id')

    # 商户当天优惠券领取次数(同一商户的优惠券在同一天被领取的数目)
    ...
    # 不同打折优惠券不使用次数
    temp = coupon_noconsume.groupby('Discount_rate').size().reset_index(name='c10')
    X = pd.merge(X, temp, how='left', on='Discount_rate')
    ...
    # 用户领取商家的优惠券后不核销次数
    temp = coupon_noconsume.groupby(['User_id', 'Merchant_id']).size().reset_index(name='um2')
    X = pd.merge(X, temp, how='left', on=['User_id', 'Merchant_id'])
    ...
    temp = coupon_noconsume.groupby('User_id').size().reset_index(name='temp')
    X = pd.merge(X, temp, how='left', on='User_id')
    X['um5'] = X.um2 / X.temp
    ...
    with ProcessPoolExecutor() as e:
        X = pd.concat(e.map(task, X_chunks, X_lists, counters))
